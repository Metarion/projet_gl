...
{	
	...
	private boolean testVertical(Cellule c)
	{
		...
		while(continu)
		{
			if(c.getCordY() - y >= 0)
			{
				if(tableau_cellule[c.getCordX()][c.getCordY() - y].isChecked() && tableau_cellule[c.getCordX()][c.getCordY() - y].checkBy() == TraitementJoueur.JOUEUR_ACTUEL)
					cpt_bon++;
				else
					continu = false;
			}
	...
	private boolean testHorizontal(Cellule c)
	{
		...
		while(continu)
		{
			if(c.getCordX() - x >= 0)
			{
				if(tableau_cellule[c.getCordX() - x][c.getCordY()].isChecked() && tableau_cellule[c.getCordX() - x][c.getCordY()].checkBy() == TraitementJoueur.JOUEUR_ACTUEL)
					cpt_bon++;
				else
					continu = false;
			}
	...
	private boolean testDiagonalGauche(Cellule c)
	{
		...
		while(continu)
		{
			...
			{
				if(tableau_cellule[c.getCordX() - x][c.getCordY() - y].isChecked() && tableau_cellule[c.getCordX() - x][c.getCordY() - y].checkBy() == TraitementJoueur.JOUEUR_ACTUEL)
					cpt_bon++;
				else
					continu = false;
			}
